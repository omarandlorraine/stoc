...
// TODO: Why does the exhaustive search yield just "php" for test4?
void search_instruction(context_t *reference, context_t *rewrite, int offs) {
    ...
    for (int op = 0; op < 256; op++) {
        if (!opcode_legal_p(op))
            continue;
        if (!is_implied_instruction(op))
            continue;
    ...
    for (int op = 0; op < 256; op++) {
        ...
        if (!opcode_legal_p(op))
            continue;
        if (!is_immediate_instruction(op))
            continue;
        rewrite->program.instructions[offs].opcode = op;
